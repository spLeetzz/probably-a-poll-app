...
export function useEventRealtime(opts: {
  ...
  useEffect(() => { cbNewRef.current = opts.onResponseNew }, [opts.onResponseNew])
  useEffect(() => { cbCountRef.current = opts.onResponseCount }, [opts.onResponseCount])
  useEffect(() => { cbParticipantRef.current = opts.onParticipantStatusUpdated }, [opts.onParticipantStatusUpdated])
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      socket.on('room:joined', () => { if (!cancelled) setStatus('live') })
      ...
      socket.on('participant:status_updated', (payload: ParticipantStatusPayload) => {
      ...
      socket.on('error', (e: { message?: string }) => {
        if (cancelled) return
        const m = e?.message ?? ''
        setStatus(m.includes('private') || m.includes('Access denied') ? 'private' : 'unavailable')
      })
      socket.on('connect_error', () => { if (!cancelled) setStatus('unavailable') })
  ...
  }, [opts.enabled, opts.eventId, opts.event?.resultsVisibility, opts.isCreator, opts.event])
